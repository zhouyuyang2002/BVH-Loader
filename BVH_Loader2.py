...
def TransMat(rotX, rotY, rotZ):
    return Rot.from_euler('xyz',[rotX,rotY,rotZ],degrees = True)
...
def draw(root, par, queue, frame):
    ...
    if root.isroot == True:
        ...
        ## 不是根节点，根据和父节点的关系
        offset = par.rot.apply(root.offset)
        ...
        root.bone, = frame.plot(arrayx,arrayy,arrayz,color='red')
        if root.isend == False:
            root.rot = TransMat(mp['Xrotation'],mp['Yrotation'],mp['Zrotation']) * par.rot
    root.joint, = frame.plot(root.coordinate[0],root.coordinate[1],root.coordinate[2],'bo')
...
def update(root, par, queue):
    ...
    if root.isroot == True:
        ...
        root.bone.set_data(np.array([[root.coordinate[0],par.coordinate[0]],
                                     [root.coordinate[1],par.coordinate[1]]]))
        root.bone.set_3d_properties(np.array([root.coordinate[2],par.coordinate[2]]))
        if root.isend == False:
            root.rot = TransMat(mp['Xrotation'],mp['Yrotation'],mp['Zrotation']) * par.rot
    root.joint.set_data(np.array([[root.coordinate[0]],
                                  [root.coordinate[1]]]))
    root.joint.set_3d_properties(np.array([root.coordinate[2]]))
                                 
    for child in root.childs:
        update(child, root, queue)
        
if __name__ ！= "__main__":
    BVH_file = "0005_2FeetJump001.bvh"
    fin = open(BVH_file, "r")
    str = fin.readlines()
    str, root = do_op(str)
    ...
    frame.set_ylim3d([0,80])
    frame.set_zlim3d([-40,40])
    frame.set_xlabel('X')
    frame.set_ylabel('Y')
    frame.set_zlabel('Z')
